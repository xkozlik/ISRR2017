...
Current methods describing the ligand passing through the tunnel are based either only on geometry of the tunnel or on molecular dynamic simulations. 
In this paper, we propose \red{TODO}
Additionally, we introduce several approaches to visualization of detected tunnels which depict valuable information about various properties and behavior of the ligand.
...
related to 
protein folding~\cite{al2012motion,gipson2012computational,amato2002using,raveh2009rapid,novinskaya2015improving,songPFintro}, analysis of loop motions~\cite{cortes2004geometric},
...
A RRT-based method for computing exit pathways for a small molecule was presented in~\cite{cortes2010simulating}.
Both protein and ligand are represented as flexible kinematic chains.
To decrease the dimension of the related configuration space, the RRT-ML variant~\cite{cortes2007mlrrt} is employed.
In RRT-ML expands the tree primarily using those DOF that are essential for achieving motion of the ligand (i.e., rotation
        and translation) and it employs the other DOFs (i.e., that are responsible for conformation change) if they hinder
the growth of the tree.
The RRT-ML is also used in the MoMa-LigPath online service.

The~\cite{cortes2010simulating} approach aims to find any exit pathway for a given ligand, which requires
...
Based on the biochemical properties, the chemists can decide if a given tunnel is relevant and can be possibly
used by a ligand to exit the protein.
