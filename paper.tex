...
Bio-chemists decide if such a tunnel can be used to transport a given ligand mainly based on the tunnel lengths and bottleneck (i.e., the radius of the smallest sphere that can traverse the whole tunnel).
This is used for example in protein engineering, where the task is to change selected properties of a protein, e.g. its stability under different outer conditions~\cite{Koudelakova2013} or activity of the protein towards the other molecules~\cite{Pavlova2009}.
...
\subsection{Main idea \red{VV dodelam}}

The analysis of tunnel traversability is based on computation of multiple trajectories of the ligand in the tunnel.
As the traversability may depends on the initial position of the ligand, especially its rotation, it is necessary to compute
...
The rotation part of $q$ and conformation index are set randomly, the scale is set to $\smin$.
If the sample $q$ is collision-free, it can be considered as a new starting configuration $\qinit$.

Then, multiple trajectories are computed starting from $\qinit$ using the method proposed in the next section.
...
from $N(c_v,\Sigma)$, where $\Sigma$ is diagonal matrix with diagonal entries equal to a parameter $\rv$, and the rotational
...
The path between $\qinit$ (root of the tree) and $q'$, where $q'$ is the neearest node in the tree towards $\qgoal$, is found.
The path $P=(q_i), q_i \in \C$ is represented by a sequence of the collision-free configurations.

Due to stochastic nature of RRT, the computed trajectories may differ in each run.
Moreover, they are influenced by the initial configuration $\qinit$, especialy by the rotation of the ligand.
To evaluate traversability of the given tunnel, multiple starting configurations are generated and for each
of these configurations, multiple trajectories are computed.
This results in a set of $n$ trajectories $P_i, i=1,\ldots,n$.
These trajectories are then used to evaluate tunnel characteristics, which is described in the next section.

\subsection{Traversability characteristics}

\red{VV is significantly rewriting this section}

There basic characteristics of the tunnel $T$ are computed from the trajectories: the accessiblity, throughput and the scaling factor.
...
We employ the tunnel representation from CAVER 3.0 and we define a tunnel part as a sphere $P = (x, r)$ where $x \in \mathbb{R}^3$ is sphere center and $r \in \R$ is radius.
