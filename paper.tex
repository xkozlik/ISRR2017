...
The above defined characteristics can be shown even as a graph, or better, presented visually by mapping then using colors to the tunnel spheres.
...
(a) & (b) \\                       
\end{tabular}
\caption{Visualization of the resulting trajectories. The tunnel begins in the top left corner.
...
Three tunnels were detected using CAVER 3.0~\cite{caver3} for the spherical probe of radius 0.9~\AA.
The traversability of the first tunnel (represented by $n=76$ spheres), was evaluated for 1-chlorpropan (denoted as $\LA$) with 11 atoms and 
...
to distance $\dt=2$~\AA) and they are considered unsuccessful otherwise.
The throughput computed from the trajectories shows that the tunnels are difficult not only around bottlenecks, but
also in other places.
The comparison of classic bottleneck and throughput is depicted in Fig.~\ref{fig::tunnel2}a.
...
Despite the low bottleneck of this tunnel, the ligand may reach its end using an alternative path way. 
...
Real proteins are dynamic structures and consequently also the tunnels are dynamic: their move, merge or even disappear due to motions of protein atoms.
Contrary, usually static tunnels are used in protein engineering, or 
...
Despite this simplification, researches claim that static tunnels help in their research.
...
or too pessimistic (e.g. the trajectory is not found because of narrow passage around the tunnel, which can in fact be
...
Having a better planner (in the term of speed or even success rate) however does not mean that the resulting trajectories will help the biochemists more.

For a daily and practical utilization of here proposed idea, it is 
necessary to compare trajectories from existing MD simulations with the trajectories obtain using motion planning and find
suitable parameters and thresholds (e.g. about the throughput) that will be used for making decisions.
...
Protein tunnels are transporting pathways for ligands towards active sites and nowadays, they traversability is evaluated
only considering one-atom ligands (probes).
In this paper, we propose to analyze traversability in tunnels using motion planning, that can consider both shape
of the protein and ligand.
We proposed modifications to RRT method to compute trajectories for a ligand represented by a library of common conformations.
The corresponding configuration space is sampled using guided sampling, where a virtual sphere moves along the tunnel and attracts growth
of the tree towards it.
The enable trajectory computation in proteins, ligands atomic radii are scaled down.
Besides classic property like bottleneck radius, the computed trajectories allows us to evaluate traversability of tunnels using
