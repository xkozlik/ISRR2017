...
The tunnel is represented by a set of spheres, that are used to guide the exploration in the search space.
Multiple trajectories are computed for the ligand being tested.
We introduce several approaches for visualization of trajectories which depict valuable information about various properties and behavior of the ligand.
...
The average success rate is compute as the average value of the success rates achived for all 50 initial configurations.

The highest average success rate is achived for the most scaled-down ligands ($\smin=0.3$) and it decreases with the increasing $\smin$. 
...
Sampling-based motion planning is a powerful tool for computing tajectories for flexible ligands.
The proposed extensions to the RRT planner have been motivated by the necessity to cope with the narrow passage
and the high dimensionaly of the flexible ligands.
...
The sampling along the tunnel centerline (in the ditance defined by $\rv$) does not only lead to better growth of the tree in the tunnel, but it also limits the maximal deflection of the trajectory from the tunnel.
...
The runtime of the planner is curently given mainly by the collision-detection.
...
A fast collision-detection can be achieved either using fast hiearchical data structures as OBB (implemented e.g. in~\cite{ozcollide})
