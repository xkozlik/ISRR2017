...
The decision based only on the spherical tunnels requires previour experties in the domain and yet it may be imprecise.

To provide chemists more insight into the behavior of non-spherical ligands, it is necessary to compute the trajecotries of the ligand considering it's shape and possibly also conformation changes, which can be formulated as a path planning problem in the a high-dimensional configuration space.
...
This requires to compute trajectorie of the non-spherical ligand in the tunnels, which can be formulated as a path planning problem in a high-dimensional configuration space $\C$.
The configuration space is formed by all possible configurations of the ligand in the tunnel, i.e., considering its rotation, translation and possibly also other degrees of freedom responsible for conformation changes.
...
The kinematic chain representaion used to model ligand flexibility allows the sampling-based planners to generate all possible conformations.
...
Moreover, the aditional degrees of freedom required to model the flexiblity increases dimension of the configuration space.
...
The approach~\cite{cortes2010simulating} aims to find any exit pathway for a given ligand, which requires
to search the whole configuration space of the ligand/protein complex.
This might be a possible disadvantage, as the proteins are dense structures with relatively narrow void space, which has the
properties of the narrow passage~\cite{hannaWIS}.
A narrow passage is a region in the configuration space whose removal changes the connectivy of the free space.
Narrow passages have smaller volume than other areas in it is therefore diffuclt to sample them dense enough using
...
We assume that the tunnel is computed using the standard tunnel detection tool and is represented using a sequence of spheres.
To guide the configuration tree through the configuration space, the random samples are generated gradually using the spheres of the tunnel.
...
The radius $R_v$ is set as the absolute distance from the tunnel centerline, e.g. $R_v=3\AA$.
...
The througput of the tunnel part $P$ is defined as a ratio of trajectories that passed $P$ over all trajectories that reached $P$.
\end{itemize}
...
\begin{figure}
\centering
\includegraphics[width=0.5\textwidth]{fig/protein-tunnel-gray}
\caption{\label{fig::tunnel}
    The protein 1CQW visualized using its secondary structure (gray) with its highest ranked tunnel (green) which was detected using CAVER 3.0.
}
\end{figure}
...
guiding, parametes, different sampling, flexibility
