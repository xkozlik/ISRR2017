...
Bio-chemists then decide if such a tunnel can be used to transport a given ligand mainly based on the tunnel lengths and its bottleneck.
...
For better decision, it is necessary to consider also the shape of the ligand being investigated, and consequently, to also
...
The most relevant paper is~\cite{cortes2005path}, where RRT is used to compute pathways for flexible ligands towards the active site, but
...
the relative volume of the free space, which increases probability of placing random sampling in the perspective regions of the configuration 
space.
...
%    $newnodes = $ false\;
    \ForEach{$s \in (\smax,\smax-\sdelta, \ldots, \smin)$}{
        \For{$i = 1,\ldots,m$}{
            ...
            $q'.l = l$\;
            $q'.s = s$\;
            $qn$ = nearest node in $\T$ towards $q'$\;
            \If{$isCollisionFree(q')$ {\bf and} $d(q',\qnear) < d(q',qn)$}{
                $\T$.addNode($q$)\;
                $\T$.addEdge($\qnear,q$)\;
%                $S = S \cup \{ q \}$\;
%                $newnodes = $ true\;
