...
Obviously, the most important is $A(n)$ of the last sphere of the tunnel, which can be considered as the overal difficulty of the tunnel.
...
The above defined characteristics can be shown even as a graph, or better, presented visually by mapping then using collors to the tunnel spheres.
...
This shows the strenght of the analysis based on the trajectories and not only on the geometric bottlenecks.
...
\caption{\label{fig::tunnel2}
    ...
    (b) Successfull (green, top) trajectories that reached end points of the tunnels,
        and unsuccessful ones (red, bottom) for $\smin=0.4$.
    (c) Successfull and unsuccessful trajectories for $\smin=0.5$.
}
}
\end{figure}
...
The achieved results have to be 
