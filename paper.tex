...
We propose several modifications to RRT in order to compute the trajectories around the tunnel and also to consider the flexibility of the ligand.
...
As the expansion routine attemts to find such sample that minimizes this distance to $\qrand$, it supports retraction
...
The scale of ligand may vary in each part of the trajectory and also the conformations.
...
% by VV ==================================================================
%For the purpose of clusting, the trajectories have to be first represented by vectors of same length.
%\def\cstart{\overline{c_{start}}}
%\def\cend{\overline{c_{end}}}
%Let $\cstart \in \R^3$ and $\cend \in \R^3$ denote the average starting and ending point of all computed trajectories.
%A line $x(t) = \cstart + t(\cend - \cstart), 0 \le t \le 1$ is created. For each point on the line, a radius
%$r(t)$ is assigned as $r(t) = 



To compare two trajectories  (which are sequences of configurations in the configuration space), they are first converted
...
%To obtain the vector $v$ for a given trajectory $P$, first a set of spheres 
 %$S_1, ..., S_M$, where $S_i = (\cstart, r_i)$ where $r_i = i \frac{|\cstart \cend|}{M + 1}$.
