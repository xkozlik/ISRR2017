...
\abstract{
%\begin{abstract}
Proteins are involved in many biochemical processes and they function is achieved by interactions with other molecules such as ligands.
The interaction takes place in an active site inside the protein, which is accesible from the surface by so called tunnels.
...
Widely used tools for tunnel detection are based on Voronoi diagrams.
This however requires to approximate the ligand by a sphere, which is not always realistic.
In this paper, we propose a motion planning technique for analysis of traversability of such tunnels by flexible ligands.
To decrease the dimension of the related configuration space, the flexibility of ligands is modeled by a set of expected conformations.
The proposed method is based on the Rapidly Exploring Random Tree motion planning.
The spheres representing the tunnel are used to guide the growth of the RRT tree in the configuration space.
...
\red{prepsat, todle uz jsme meli vickrat}
...
These methods compute the tunnels assuming a spherical ligand (probe).
The resulting tunnels are represented as a sequence of spheres.
...
Bio-chemicists then decide if such a tunnel can be used to transport a given ligand mainly based on the tunnel lengths and its bottleneck.

\red{TODO}
something about screeing: what is it, what's is the input and output, in which applications (research/industry) it is useful.


Such decision is however not precise and it requires previous expertise with the domain.
Of the reasons is that typical ligands are non-spherical and therefore it is difficult to estimate their traversability in tunnels
computed for a spherical probe.
For better decision, it is necessary to consider also the shape of the ligand being investigaed, and consequently, to also
consider rotations of the whole ligand.
Moreover, ligands are rather flexible molecules, so the relative positions of their atoms can change as well.

Sampling-based motion planning methods such as Rapidly Exploring Random Tree~\cite{lavalleRRT} can be
used to test the traversability of a given ligand in the tunnels.
These methods can cope with objects of many degrees of freedom and arbitrary geometry.
...
Finding tunnels for a general, non-spherical and possibly a flexible ligand can be solved by searching
a feasible path in the corresponding configuration space $\C$.
The dimension of the space is given by the degrees of freedom (6D for a solid ligand and 6D+$n$ for a flexible ligand with
$n$ rotational bonds).
...
to a goal configuration $\qgoal\in\CF$, where both $\qinit$ and $\qgoal$ are located at the begining and the end of the
tunnel, respectively.
Sampling-based methods like PRM~\cite{kavrakiPRM} or RRT~\cite{lavalleRRT} can be used to search such a path.
%Sampling-based techniques like PRM~\cite{kavrakiPRM} or RRT~\cite{lavalleRRT} can be used to search such a high-dimensional space.
%The tunnel detection in dynamic proteins can also be formulated as path planning~\cite{vonasek2016application}. in 4D (i.e., 3D $\times$ time) 
...
The most relevant paper is~\cite{guieysse2008structure}, where RRT is used to compute pathways for flexible ligands towards the active site, but
without the possibility to select a preferable path/tunnel.
...
%Despite the flexibility of ligands, their movements in proteins are strongly limited, as they are typically larger than spherical probes.
...
In this paper, we propose to employ motion planning to compute the trajectory for a given ligand in a given tunnel.
This brings several advantages.
From motion planning point of view, the tunnels have properties of narrow passages.
By focusing the motion planning only to the surrounding of a tunnel, the total volume of the configuration space decreases which increases
the relative volume of the free space, which increases probability of placing random sampling in the perspective regions of the configruation 
space.
...
and for motion planning of rigin objects in amongst 
...
To decrease the computational burgen, a set of feasible conformations $\L$ is prepared, e.g. using Rosseta tool.
...
The configuration space is searched using the RRT principle~\cite{lavalleRRT}.
...
and the node is expanded.
The random samples are not generatd only from whole $\C$, as in the original RRT, but the sampling
is guided using moving virtual goal~\cite{vonasek2009rrt}.
The virtual goal $v$ is the index of the actual sphere of the tunnel, around which the samples
are generated with probability $\gb$. 
...
The algorithm termines after a predefined planning trials $\Imax$ or if the tree reaches
...
\While{$iteration < \Imax$ and $v < N$}{
    ...
    $S = $expandConfigurations($\qnear,\L$)\;
...
The core of the method is the expansion procedure, which generates new collision-free nodes around $\qnear\in\T$.
The algorithm is listed in Alg.~\ref{alg::expand}.
For each conformation $l \in \L$, the expansion generates up to $m$ samples $q$ with maximal radii around $\qnear$.
...
\ForEach{$l \in \L$}{
    \ForEach{$s \in (\smax,\smax-\sdelta, \ldots, \smin)$}{
        \For{$i = 1,\ldots,m$}{
            $q' = q$\;
            translate $q$ by random around $q'$\;
            rotate randomly \;
            $q'.l = l$\;
            $q'.s = s$\;
...
Despite, a trajectory leading as close as possible towards $\qgoal$ is extracted fromm the tree.
