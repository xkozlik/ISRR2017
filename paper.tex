...
In this paper, we propose to evaluate the tunnels using based on ligand trajectories computed by motion planning, namely using
Rapidly Exploring Random Tree (RRT).
We propose novel modifications to RRT in order to compute the trajectories around the tunnel and also to consider the flexibility of the ligand.
We also introduce several approaches for visualization of the resulting trajectories which depict valuable information about various properties and behavior of the ligand.
...
The analysis of tunnel traversability is based on computation of multiple trajectories of the ligand inside the tunnel.
As the traversability may depend on the initial position of the ligand, especially its rotation, it is necessary to compute
the trajectories from multiple starting configurations $\qinit$.
The starting configuration $\qinit$ is generated by randomly sampling the vicinity of the first sphere $c_1$ of the tunnel.
A sample $q$ is created by generating its translation part randomly around $t_1$ in the distance $R$.
The rotation part of $q$ and the conformation index are set randomly, the scale is set to $\smin$.
If the sample $q$ is collision-free, it can be considered as a new starting configuration $\qinit$.
Then, multiple trajectories are computed starting from $\qinit$ using the method proposed in the following section.

\subsection{RRT for Ligand Trajectory Computation}
...
\KwIn{
    tunnel spheres $c_i$ and their radii $r_i$, $i=1,\ldots,n$, 
...
\While{$iteration < \Imax$ {\bf and}  $v < n$}{
    \eIf{$rand() < \gb$}{
        $\qrand$ = random sample around actual virtual goal $c_n \in T$\;
    }{
...
The expansion begins with the largest scale $s$ and if less than $m$ collision-free samples are generated with $s$, the next lower $s$ is used.
...
The result of the planner is the tree $\T$ of collision-free configurations.
The path between $\qinit$ (root of the tree) and $q'$, where $q'$ is the nearest node in the tree towards $\qgoal$, is found.
The path $P=(q_i), q_i \in \C$ is represented as a sequence of collision-free configurations.
Let $d(P,i)$ denote the 3D Euclidean distance.

The path $P$ reaches a particular sphere $t_i$ in the tunnel if the 3D Euclidean distance between the sphere and its nearest configuration
in the path is less than the threshold $T$.
TODO VV dodelam
...
The traversability of the tunnel $T$ is evaluated using all computed trajectories $P_i$. 
Three basic characteristics of the tunnel are computed from the trajectories: the accessiblity, throughput and the scaling factor.
These characteristics are computed for each sphere $(c_i,r_i) \in T$ of the tunnel.
This requires to determine which parts of the tunnel were accessed by the trajectories.
...
As described above, our approach enables to estimate the probability that the ligand passes through the tunnel.
...
In order to further support the tunnel exploration process performed by biochemists.
To do that, we employ an ensemble of resulting trajectories $T$, and we compute and visualize the following tunnel properties:
\begin{itemize}

\item \textbf{Ligand scale} --- The scale of the ligand when it passes a given tunnel's part.
...
Similarly, the minimum and maximum ligand scales can be defined.
  \item \textbf{Throughput} --- The probability that the ligand passes through a tunnel's part.
...
\end{itemize}
In the visualization, these properties are mapped onto the tunnel's surface (see Fig.~\ref{fig:properties}) so that they can be visually correlated with, e.g., the tunnel's shape.
...
We define the similarity between two trajectories as follows.
...
Then, a vector $v$ for a trajectory $t$ is obtained such that an $i$-th sample $x_i$ in $v$ corresponds to the position in $t$ which is the nearest one to the surface of $S_i$.
...
Left: All trajectories colored according to whether they reached the end of the tunnel (green) or not (red).
