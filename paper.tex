...
\While{$iteration < \Imax$ {\bf and}  $v < N$}{
   ...
   % \For{$q \in S$}{
    ...
    \For{$i= N,N-1,\ldots v+1,v$}{ \nllabel{alg::main:a}
        $d$ = nearest node in the tree to $T[i]$\;
        \If{$d < \dt $}{
...
For each ligand conformation, the expansion procedure attempts to find a new collision-free configuration around $\qnear$ with maximal
scale.
For a given conformation $l \in \L$, first the maximal scale $\smax$ is considered and
$m$ samples are generated around $\qrand$ and tested for collision.
The nearest collision-free sample towards $\qrand$ is selected and added to the tree.
If none of the samples is collision-free, the scale is reduced to $\smax-\sdelta$ and the searches continues
